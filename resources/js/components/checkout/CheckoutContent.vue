...
const missingFields = computed(() => {
    ...
    // Address only for purchases
    if (purchaseType.value === 'purchase' && !formData.value.address) {
        missing.push('Delivery Address');
    }

    // Beneficiary (optional, only check if user opened and kept it on)
    if (purchaseType.value === 'donation' && showBeneficiaryForm.value && formData.value.has_beneficiary) {
        if (beneficiaryIsOrganization.value) {
            if (!formData.value.beneficiary_organization_name) {
                missing.push('Organization Name');
            }
        } else {
            // For individuals: require at least one of first or last name
            if (!formData.value.beneficiary_first_name && !formData.value.beneficiary_last_name) {
                missing.push('Beneficiary First or Last Name');
            }
        }
    }
...
<!-- Missing fields helper (outside the button) -->
<div
